 ...
 */
export type TokenType = 'LAMPORTS' | 'USDC' | 'USDT';
...
export function getTokenType(t: bigint | number): TokenType {
    if (t === BigInt(0) || t === 0) return 'LAMPORTS';
    if (t === BigInt(1) || t === 1) return 'USDC';
    if (t === BigInt(2) || t === 2) return 'USDT';
    throw new Error(INVALID_TOKEN_TYPE);
}

export function getNumberFromTokenType(t: TokenType): number {
    if (t === 'LAMPORTS') return 0;
    if (t === 'USDC') return 1;
    if (t === 'USDT') return 2;
    throw new Error(INVALID_TOKEN_TYPE);
}
...
export function getTokenTypeFromStr(t: string): TokenType {
    if (t === 'LAMPORTS' || t === 'USDC' || t === 'USDT') return t;
    throw new Error(INVALID_TOKEN_TYPE);
}
